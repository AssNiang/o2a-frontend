...
import { HealthStructService } from 'src/app/shared/services/health-struct.service';
...
export class StructureLocationComponent implements OnInit {
  ...
  constructor(
    ...
    private _healthStructService: HealthStructService
  ) {}
  ...
  ngOnInit(): void {
    ...
    navigator.geolocation.getCurrentPosition(
      (position: GeolocationPosition) => {
        ...
        this._healthStructService
          .getAllHealth_struct()
          .subscribe((health_struct) => {
            ...
            for (let structure of health_struct) {
              let circle = L.circle([structure.latitude, structure.longitude], {
                color: 'red',
                fillColor: '#f03',
                fillOpacity: 0.5,
                radius: 100,
              ...
              circle.bindPopup(
                `${structure.name} <br> Latitude: ${structure.latitude} <br> Longitude: ${structure.longitude}`
              );
            }
          });
  ...
  submit(health_struct: NgForm) {
    try {
      this._healthStructService
        .addHealth_struct(health_struct.value)
        .subscribe((h_struct) => {
